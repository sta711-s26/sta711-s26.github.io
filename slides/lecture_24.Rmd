---
title: "Hypothesis testing framework"
author: "Ciaran Evans"
output: beamer_presentation
---

## General framework for hypothesis tests

**Definition:** Let $\theta \in \mathbb{R}^p$ be some parameter of interest. A **hypothesis** is a statement about $\theta$. The goal of a hypothesis test is to compare two competing hypotheses:

$$H_0: \theta \in \Theta_0 \hspace{1cm} H_A: \theta \in \Theta_1$$

\vspace{5cm}


## Outcomes

$$H_0: \theta \in \Theta_0 \hspace{1cm} H_A: \theta \in \Theta_1$$

The test either **rejects** $H_0$ or **fails to reject** $H_0$. Possible outcomes:

\vspace{5cm}

## Goal

| | $H_0$ is true | $H_0$ is false |
| --- | --- | --- |
| fail to reject | correct decision | type II error |
| reject | type I error | correct decision |


**Goal:** Minimize type II error rate, subject to control of type I error rate 

## Constructing a test

$$H_0: \theta \in \Theta_0 \hspace{1cm} H_A: \theta \in \Theta_1$$

\vspace{5cm}

## Constructing a test

Given observed data $X_1,...,X_n$:

1. Calculate a test statistic $T_n = T(X_1,...,X_n)$
2. Choose a rejection region $\mathcal{R} = \{(x_1,...,x_n) : \text{ reject } H_0\}$
3. Reject $H_0$ if $(X_1,...,X_n) \in \mathcal{R}$

**Example:** $X_1,...,X_n$ iid with mean $\mu$ and variance $\sigma^2$

\vspace{3cm}

## Power function

Suppose we reject $H_0$ when $(X_1,...,X_n) \in R$. The **power function** $\beta(\theta)$ is
$$\beta(\theta) = P_{\theta}((X_1,...,X_n) \in R)$$

\vspace{5cm}


## Example

$X_1,...,X_n$ iid from a population with mean $\mu$ and variance $\sigma^2$. 

$$H_0: \mu = \mu_0 \hspace{1cm} H_A: \mu \neq \mu_0$$

\vspace{5cm}

## Example

$$H_0: \mu = \mu_0 \hspace{1cm} H_A: \mu \neq \mu_0$$
$$\beta(\mu) \approx 1 - \Phi\left(z_{\alpha/2} - \left(\frac{\mu- \mu_0}{\sigma/\sqrt{n}} \right) \right) + \Phi\left(-z_{\alpha/2} - \left(\frac{\mu- \mu_0}{\sigma/\sqrt{n}} \right) \right)$$
Example: $\sigma = 1$, $\mu_0 = 0$

```{r, echo=F, message=F, warning=F, fig.align='center', fig.width=4.5, fig.height=2.25}
library(tidyverse)
mu <- seq(-3, 3, 0.01)
z_a2 <- 1.96
s <- 1
beta <- c()
for(n in c(15, 30, 50)){
  beta <- c(beta, 1 - pnorm(z_a2 - mu/(s/sqrt(n))) + pnorm(-z_a2 - mu/(s/sqrt(n))))
}

data.frame(beta, n = as.factor(rep(c(15, 30, 50), each=length(mu))), 
           mu = rep(mu, 3)) |>
  ggplot(aes(x = mu, y = beta, color = n)) +
  geom_line() +
  geom_abline(slope = 0, intercept = 0.05, lty = 2) +
  theme_bw() +
  labs(y = "Power")

```



## Rejecting $H_0$

$$H_0: \theta \in \Theta_0 \hspace{1cm} H_A: \theta \in \Theta_1$$

**Question:** A hypothesis test rejects $H_0$ if $(X_1,...,X_n)$ is in the rejection region $\mathcal{R}$. Are there any issues if we only use a rejection region to test hypotheses?

\vspace{4cm}


## p-values

$$H_0: \theta \in \Theta_0 \hspace{1cm} H_A: \theta \in \Theta_1$$

A **p-value** $p(X_1,...,X_n)$ is a test statistic satisfying:

\vspace{6cm}

## Example: p-values

Generate data $X_1,...,X_{20} \overset{iid}{\sim} N(\mu, 1)$. Test $H_0: \mu = 0$ vs. $H_A: \mu \neq 0$.

Distribution of p-values:

```{r, echo=F, message=F, warning=F, fig.align='center', fig.width=4.5, fig.height=2.25}
p_vals_0 <- rep(NA, 1000)
mu <- 0
for(i in 1:1000){
  x <- rnorm(20, mu, 1)
  p_vals_0[i] <- 2*pnorm(abs(mean(x)*sqrt(20)), lower.tail=F)
}

p_vals_1 <- rep(NA, 1000)
mu <- 0.3
for(i in 1:1000){
  x <- rnorm(20, mu, 1)
  p_vals_1[i] <- 2*pnorm(abs(mean(x)*sqrt(20)), lower.tail=F)
}

data.frame(p = c(p_vals_0, p_vals_1),
           mu = paste0("mu = ", rep(c(0, 0.3), each=1000))) |>
  ggplot(aes(x = p)) +
  geom_histogram(bins = 20) +
  theme_bw() +
  facet_wrap(~mu)
```



## p-values

Suppose we have a test which rejects $H_0$ when $T(X_1,...,X_n) > c_\alpha$, where $c_\alpha$ is chosen so that

$$\sup \limits_{\theta \in \Theta_0} \beta(\theta) = \sup \limits_{\theta \in \Theta_0} P_\theta(T(X_1,...,X_n) > c_\alpha) \leq \alpha$$
Let $x_1,...,x_n$ be a set of observed data. 

**Theorem:** Let 
$$p(x_1,...,x_n) = \sup \limits_{\theta \in \Theta_0} P_\theta(T(X_1,...,X_n) > T(x_1,...,x_n))$$
Then $p$ is a valid p-value.

\vspace{3cm}

## Proof of theorem


