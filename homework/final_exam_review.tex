...
$$\boldsymbol{\theta}_2 = \boldsymbol{\theta}_1 + \begin{pmatrix}
...
Suppose we wish to test $H_{0,1},...,H_{0,m}$, all of which are truly null, and the p-values have a joint distribution that can be described as follows. Let $J \in \{0,1,...,m\}$ be a random variable with $P(J = 0) = 1 - \alpha$, $P(J = i) = \alpha/m$ for $i = 1,...,m$, and let $p_i \in [0, 1]$ depend on $J$:
